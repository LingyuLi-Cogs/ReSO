...
def generate_alphabet_ascii(target_word, ascii_coding='5x5 grid'):
    ...
    if ascii_coding == '5x5 grid':
        ...
        target_word_list = [
            char for char in str(target_word) if char.upper() in ascii_dict
        ]
        if not target_word_list:
            raise ValueError("ArtPrompt word contains no renderable ASCII characters")
        ascii_codes = []
        ...
        for i in range(len(target_word_list)):
            c_ascii = [
                row.replace('*', target_word_list[i])
                for row in ascii_dict[target_word_list[i].upper()]
            ]
