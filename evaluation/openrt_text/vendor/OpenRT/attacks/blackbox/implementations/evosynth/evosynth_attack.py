...
class EvosynthConfig:
    ...
    # Bound each Agents SDK run so local 32K-context helpers cannot accumulate
    # an unbounded tool transcript. Partial phase state is retained when a
    # phase reaches its budget.
    reconnaissance_max_turns: int = 8
    tool_creation_max_turns: int = 8
    exploitation_max_turns: int = 8
    coordinator_max_turns: int = 4
    max_generated_tools: int = 2
    ...
    def __post_init__(self):
        ...
        turn_budgets = (
            self.reconnaissance_max_turns,
            self.tool_creation_max_turns,
            self.exploitation_max_turns,
            self.coordinator_max_turns,
        )
        if any(value <= 0 for value in turn_budgets):
            raise ValueError("EvoSynth phase turn budgets must be positive")
        if self.max_generated_tools <= 0:
            raise ValueError("max_generated_tools must be positive")
...
class EvosynthAttack(BaseAttack):
    ...
    def _setup_orchestrator(self):
        ...
        if urlparse(base_url).hostname not in {"127.0.0.1", "localhost", "::1"}:
            raise RuntimeError(
                f"Offline EvoSynth refuses non-loopback endpoint: {base_url}"
            )

        # Generated tools use the legacy environment-based helper while the
        # Agents SDK uses the injected client below. Keep both paths on the
        # exact same local model identity; never fall back to an upstream
        # example model such as ``deepseek-chat``.
        os.environ["OPENAI_API_KEY"] = api_key
        os.environ["OPENAI_KEY"] = api_key
        os.environ["OPENAI_BASE_URL"] = base_url
        os.environ["OPENAI_MODEL"] = self.config.attack_model_base
        os.environ["DEFAULT_MODEL"] = self.config.attack_model_base
        ...
        orchestrator_config = {
            ...
            'attack_model_config': {
                'api_key': api_key,
                'base_url': base_url,
                'model_name': self.config.attack_model_base,
                'temperature': 0.7,
            },
            'phase_max_turns': {
                'reconnaissance': self.config.reconnaissance_max_turns,
                'tool_creation': self.config.tool_creation_max_turns,
                'exploitation': self.config.exploitation_max_turns,
                'coordinator': self.config.coordinator_max_turns,
            },
            'max_generated_tools': self.config.max_generated_tools,
    ...
    def _convert_to_attack_result(self, session_result: dict, query: str) -> AttackResult:
        ...
        final_prompt = query
        history = []
        query_result = session_results.get('query_results', {}).get(query, {})
        attack_history = []
        if context:
            attack_history = getattr(context, 'session_data', {}).get(
                'attack_history', []
            )
        query_attacks = [
            attack for attack in attack_history
            if attack.get("original_query", "") == query
        ]

        # Upstream used to export turns only after a score-5 success.  Failed
        # attacks are still valid evaluation samples, so retain every target
        # response and choose the highest-scoring turn as the final candidate.
        best_turn = None
        best_score = float('-inf')
        for attack in query_attacks:
            conversation_result = attack.get('multi_turn_results', {})
            conversation_history = conversation_result.get(
                'conversation_history', []
            )
            for turn in conversation_history:
                turn_prompt = (
                    turn.get('attack_prompt')
                    or turn.get('attack_response')
                    or query
                )
                turn_response = turn.get('target_response', '')
                turn_score = turn.get('judge_score', 0)
                try:
                    numeric_turn_score = float(turn_score or 0)
                except (TypeError, ValueError):
                    numeric_turn_score = 0.0
                history.append({
                    'prompt': turn_prompt,
                ...
                if turn_response and numeric_turn_score >= best_score:
                    best_turn = (turn_prompt, turn_response)
                    best_score = numeric_turn_score

        # Tool creation/refinement also executes genuine attack prompts
        # against the target.  The exploitation agent may elect not to run
        # when all freshly created tools score poorly, so retain these probe
        # responses as valid EvoSynth candidates as well.
        if context:
            created_tools = getattr(context, 'created_tools', [])
            if isinstance(created_tools, dict):
                created_tools = created_tools.values()
            for tool in created_tools:
                performance = getattr(tool, 'performance', None)
                for probe in getattr(performance, 'attack_results', []) or []:
                    if probe.get('original_query', '') != query:
                        continue
                    probe_prompt = probe.get('attack_prompt') or query
                    probe_response = probe.get('target_response', '')
                    probe_score = probe.get('judge_score', 0)
                    try:
                        numeric_probe_score = float(probe_score or 0)
                    except (TypeError, ValueError):
                        numeric_probe_score = 0.0
                    history.append({
                        'prompt': probe_prompt,
                        'response': probe_response,
                        'judge_score': probe_score,
                        'method': 'evosynth_tool_probe',
                        'stage': probe.get('stage', 'tool_creation'),
                    })
                    if probe_response and numeric_probe_score >= best_score:
                        best_turn = (probe_prompt, probe_response)
                        best_score = numeric_probe_score

        if best_turn:
            final_prompt, final_response = best_turn
            success = success or best_score >= self.config.success_threshold
        else:
            # Preserve phase errors when the local endpoint never managed to
            # execute a target call; the runner can then report a real error
            # rather than an opaque empty EvoSynth record.
            def safe_diagnostic(value, depth=0):
                ...
                if isinstance(value, dict):
                    return {
                        str(key): safe_diagnostic(item, depth + 1)
                        for key, item in value.items()
                        if key != 'result_summary'
                    }
                if isinstance(value, (list, tuple)):
                    return [safe_diagnostic(item, depth + 1) for item in value]
                return f"<{type(value).__name__}>"

            history.append({
                'orchestrator_result': safe_diagnostic(query_result),
                'method': 'evosynth_multi_agent_diagnostics'
            })
